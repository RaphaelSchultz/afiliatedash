-- Fix commission calculation: use SUM instead of MAX to capture all items in order
...
WITH filtered_vendas AS (
  ...
  -- Comissão: SUM por order_id para capturar todos os itens do pedido
...
-- Fix get_relatorio_diario_network to use SUM for commission
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(total_commission), 0) as comissao_liquida,
