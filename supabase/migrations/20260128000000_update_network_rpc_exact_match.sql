...
BEGIN
  ...
  WITH filtered_vendas AS (
    ...
    WHERE user_id = auth.uid()
      ...
      -- No status filter, matching System A
  ),
  
  -- Match System A Logic: Group by ORDER_ID first
  order_commissions AS (
    SELECT 
      data_d,
      order_id,
      -- System A uses MAX for commission (implying row commission might be total or duplicated)
      MAX(commission) as max_commission,
      -- System A uses SUM for amount (implying actual_amount is split per row)
