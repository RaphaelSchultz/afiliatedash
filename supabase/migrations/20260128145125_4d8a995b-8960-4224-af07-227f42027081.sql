...
-- Fix: Usar MAX para net_commission em vez de SUM para evitar duplicação
-- A Shopee reporta a comissão total do pedido em CADA linha de item
...
BEGIN
  ...
  -- Aggregate by order_id: SUM for GMV (all items), MAX for commission (avoid duplication)
  order_totals AS (
    SELECT 
      fv.order_id,
      SUM(COALESCE(fv.actual_amount, 0)) as order_gmv,
      MAX(COALESCE(fv.commission, 0)) as order_commission  -- MAX instead of SUM!
