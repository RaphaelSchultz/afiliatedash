...
-- Fix status filter to consider BOTH status and order_status columns
-- Some records have status NULL but order_status = 'Cancelado'
...
BEGIN
  ...
  -- Aggregate by order_id: SUM for GMV, SUM for commission (corrected aggregation)
  order_totals AS (
    SELECT 
      fv.order_id,
