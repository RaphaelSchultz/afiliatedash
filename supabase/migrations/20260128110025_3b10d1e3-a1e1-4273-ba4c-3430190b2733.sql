-- Fix commission calculation to use MAX per order (not SUM)
-- This matches the correct Shopee aggregation logic
...
BEGIN
  ...
  -- Aggregate by order_id: SUM for GMV, MAX for commission (to avoid duplicates)
  order_totals AS (
    SELECT 
      fv.order_id,
      SUM(COALESCE(fv.actual_amount, 0)) as order_gmv,
      MAX(COALESCE(fv.commission, 0)) as order_commission
